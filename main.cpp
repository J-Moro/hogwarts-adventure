...
// Inclua todos os cabeçalhos das classes que você está usando
...
// Função auxiliar para exibir os dados da cena de combate
void exibirDadosDaCenaDeCombate(CenaDeCombate* cena) {
    ...
    // Acessa o monstro da cena
    Monstro* monstro = cena->getMonstro(); // Assumindo que você tem um getMonstro()
...
int main() {
    ...
    // Faz um cast seguro para CenaDeCombate
    CenaDeCombate* cenaDeCombate = dynamic_cast<CenaDeCombate*>(cenaAtual);
    
    // Chama a função auxiliar para imprimir os dados
